...
CONFIGDIR="/home/apps/SiteMonitor/etc"
...
for s in $(cat $SITELIST); do
    ...
    site=$CONFIGDIR/$s
    $INSTALLDIR/bin/wget.sh "$CONFIGFILE" "$site"  >> $LOGDIR/$LOGFILE 2>&1 
    $INSTALLDIR/bin/check.sh "$CONFIGFILE" "$site"  >> $LOGDIR/$LOGFILE 2>&1 
    ERROR=$?
    #analyze results
    if [ "$ERROR" != "0" ];then
